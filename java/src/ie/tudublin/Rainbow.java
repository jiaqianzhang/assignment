...
public class Rainbow<color> extends PApplet
{
    ...
    public void drawSun()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            pushMatrix();
            rotateY(angle + frameCount * 0.01f);
    ...
    public void drawSeawaves()
    {
        ...
        for (int j = 0; j < 8; j++) // add 8 waves
        {
            ...
            {
                curveVertex(i, height - 50 * sin(i * 0.01f + frameCount * 0.05f + waveOffset)); // adjust wave height
            }
            ...
            {
                curveVertex(i, height - 50 * sin(i * 0.01f + frameCount * 0.05f + waveOffset) - 10 * cos(i * 0.02f + frameCount * 0.07f + waveOffset)); // adjust foam height and width
            }
            curveVertex(width, height);
            curveVertex(width, height);
            endShape();
    ...
    public void drawLightning()
    {
        ...
        if (random(1) < lightningProbability)
        {
            for (int i = 0; i < 5; i++)
            {
                ...
                for (int j = 0; j < 50; j++)
                {
                    float x = lerp(startX, endX, (float) (j / 30.0));
                    float y = lerp(startY, endY, (float) (j / 30.0)) + random(-50, 50);
                    int c = color(map(j, 0, 30, 255, 0), map(j, 0, 30, 0, 255), map(j, 0, 30, 128, 255)); // rainbow colors                        strokeWeight(thickness);
